/*
Package bit_l implements the bit_l protocol of BitFT,
...
type Node struct {
	...
	pendingDBlocksQC map[string]*Block                       // map from hash to the data block with QC
	pendingDBlocks   map[string]*Block                       // map from hash to the data block
	partialSigs      map[uint64]map[string]map[string][]byte // map from height to string(hash) to the partial signatures
	partialDSigs	 map[string]map[string][]byte			 // map from hash to sender to the partial signatures
	qcCreated        map[uint64]bool                         // mark if we has created a QC for the block(height)
	qcDCreated       map[string]bool						 // mark if we has created a QC for the data block(hash)
	...
	evaluation []int64 // store the latency of committed data blocks
	blockNum   int    // the total number of committed data blocks
}
...
func NewNode(conf *config.Config) *Node {
	...
	n.pendingDBlocksQC = make(map[string]*Block)
	n.pendingDBlocks = make(map[string]*Block)
	n.partialSigs = make(map[uint64]map[string]map[string][]byte)
	n.partialDSigs = make(map[string]map[string][]byte)
	n.qcCreated = make(map[uint64]bool)
	n.qcDCreated = make(map[string]bool)
...
func (n *Node) ProposeBlockLoop() {
		...
		case newHeight := <-n.candidateHeightUpdated:
			...
			for {
				if nextHeight <= n.candidateChain.height {
					block := NewBlock(n.name, nextHeight, nil, nil)
					block.IsDBlock = true
					...
					n.logger.Debug("Propose a new data block", "Node", n.name, "blockHeight", block.Height,
				...
				if n.isSortition[seedForSortition] == false {
					...
					if win {
						...
						block.IsDBlock = false
						n.lock.Lock()
						// link the data blocks which have 2f+1 votes
						for hash, _ := range n.pendingDBlocksQC {
							block.DBlockList = append(block.DBlockList, hash)
							delete(n.pendingDBlocksQC, hash)
						}
						n.lock.Unlock()
						blockAsBytes, err := encode(block)
						if err != nil {
	...
	throughPut := float64(n.blockNum * n.batchSize)/pastTime
	totalTime := int64(0)
			...
			if hashAsString == hex.EncodeToString(previousHash) {
				...
				for i := previousHeight; i > n.chain.height; i-- {
					n.logger.Info("commit a block from candidate chain to the chain", "node",
						n.name, "height", i, "link", len(block.DBlockList))
					commitTime := time.Now().UnixNano()
					n.blockNum +=len(block.DBlockList)
					for _, dHash := range block.DBlockList {
						dBlock := n.pendingDBlocks[dHash]
						latency := commitTime - dBlock.TimeStamp
						n.evaluation = append(n.evaluation, latency)
						delete(n.pendingDBlocksQC, dHash)
					}
...
func (n *Node) tryExtendChain() {
	...
	if blockWithQC, ok := n.pendingBlocksQC[newHeight]; ok {
		...
		n.logger.Info("commit the block", "node", n.name, "height", newHeight, "block-proposer", blockWithQC.Proposer,
			"link", len(blockWithQC.DBlockList))
		commitTime := time.Now().UnixNano()
		n.blockNum += len(blockWithQC.DBlockList)
		for _, dHash := range blockWithQC.DBlockList {
			dBlock := n.pendingDBlocks[dHash]
			latency := commitTime - dBlock.TimeStamp
			n.evaluation = append(n.evaluation, latency)
			delete(n.pendingDBlocksQC, dHash)
...
func (n *Node) BroadcastVote(block *Block) error {
	n.lock.RLock()
	if n.voted[block.Height] && !block.IsDBlock {
	...
	// mark the vote is for data block or not
	if block.IsDBlock {
		plainVote.IsDBlock = true
	} else {
		plainVote.IsDBlock = false
	}
	...
	} else if block.IsDBlock {
		// the vote for data block do not need RBC
		n.logger.Debug("vote the data block", "voter", n.name, "height", block.Height, "block-proposer", block.Proposer)
...
func (n *Node) handleNewBlock(block *Block) {
	...
	if !block.IsDBlock {
		...
		hash, _ := block.getHashAsString()
		n.pendingDBlocks[hash] = block
		go n.checkDIfQuorumVotes(hash)
	}
...
func (n *Node) handleVoteMsg(vote *VoteWithPartialSig) {
	...
	if !vote.IsDBlock {
		...
		hashString := hex.EncodeToString(vote.BlockHash)
		if _, ok := n.partialDSigs[hashString]; !ok {
			n.partialDSigs[hashString] = make(map[string][]byte)
		}
		n.partialDSigs[hashString][vote.Sender] = vote.PartialSig
		go n.checkDIfQuorumVotes(hashString)
	}
...
//checks if there are votes of quorum for a data block.
func (n *Node) checkDIfQuorumVotes(hash string) {
	n.lock.Lock()
	parSigs, _ := n.partialDSigs[hash]
	_, ok := n.pendingDBlocks[hash]
	defer n.lock.Unlock()
	if len(parSigs) >= n.quorumNum && !n.qcDCreated[hash] && ok {
		n.qcDCreated[hash] = true
		n.pendingDBlocksQC[hash] = n.pendingDBlocks[hash]
	}
}
